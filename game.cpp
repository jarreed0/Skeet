...
void Game :: advanceBird()
{
   if (bird == NULL)
   {
      ...
      if (random(0, 30) == 0)
      {
         ...
         bird = createBird();
         Point bPoint;
         bPoint.setCoord(topLeft.getX(), random(-150, 150));
         if(bPoint.getY() > 0) {
            bird->fire(bPoint, 225, bird->getVel().getSpeed());
         } else {
            bird->fire(bPoint, 135, bird->getVel().getSpeed());
         }
      }
   }
   else
...
Bird* Game :: createBird()
{
   ...
   //newBird->fire(p, 45, newBird->getVel().getSpeed());
...
void Game :: handleInput(const Interface & ui)
{
   ...
   {
      Bullet newBullet;
      newBullet.fire(rifle.getPoint(), rifle.getAngle(), newBullet.getVel().getSpeed());
